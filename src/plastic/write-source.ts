    ...
    parameters: {
      ...
      properties: {
        path: {
          type: "string",
          description:
            "File path relative to src/ for .ts files (e.g. 'main.ts') or relative to dist/ for .js files (e.g. 'main.js')",
        },
        content: {
          type: "string",
...
export function handleWriteSource(args: { path: string; content: string }): string {
...
function handleTSWrite(tsPath: string, content: string): string {
  ...
  try {
    ...
    if (existsSync(jsTargetPath)) {
      try { copyFileSync(jsTargetPath, jsTargetPath + ".bak"); } catch {}
    }
  ...
  try { updateManifest(tsPath); } catch {}
...
function handleJSWrite(jsPath: string, content: string): string {
