...
export function createDefaultFactory(): SensoryViewFactory {
  ...
  factory.register(
    { name: "time", maxTokens: 200, width: 80, height: 22, enabled: true, updateMode: "every_turn", viewable: true },
    () => new TemporalSource(),
  );

  factory.register(
    { name: "tasks", maxTokens: 150, width: 48, height: 12, enabled: false, updateMode: "every_turn", viewable: true },
    () => new TaskSource(),
  );

  factory.register(
    { name: "social", maxTokens: 200, width: 48, height: 12, enabled: true, updateMode: "every_turn", viewable: true },
    () => new SocialSource(),
  );

  factory.register(
    { name: "spend", maxTokens: 100, width: 48, height: 12, enabled: false, updateMode: "every_turn", viewable: true },
  ...
  factory.register(
    { name: "config", maxTokens: 120, width: 80, height: 17, enabled: true, updateMode: "every_turn", viewable: true },
    () => new ConfigSource(),
  );
