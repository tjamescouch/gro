...
export class AnthropicBatchClient {
    ...
    async submitBatch(requests) {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                throw groError("batch_error", `Batch submission failed: ${res.status} ${text}`);
            }
    ...
    async getBatchStatus(batchId) {
        ...
        try {
            ...
            if (!res.ok) {
                const text = await res.text();
                throw groError("batch_error", `Batch status fetch failed: ${res.status} ${text}`);
            }
     ...
     */
    async downloadResults(resultsUrl) {
        const headers = {
            "anthropic-version": "2023-06-01",
            "x-api-key": this.apiKey,
        };
        try {
            ...
            if (!res.ok) {
                const text = await res.text();
                throw groError("batch_error", `Results download failed: ${res.status} ${text}`);
            }
            const text = await res.text();
    ...
    async cancelBatch(batchId) {
        ...
        try {
            ...
            if (!res.ok) {
                const text = await res.text();
                throw groError("batch_error", `Batch cancel failed: ${res.status} ${text}`);
            }
