 ...
 */
const CONNECTION_ERROR_RE = /fetch timeout|fetch failed|ECONNREFUSED|ECONNRESET|ETIMEDOUT|ENETUNREACH|EAI_AGAIN|socket hang up|network error/i;
export function isConnectionError(err) {
    ...
    const causeMsg = err?.cause_message;
    if (typeof causeMsg === "string" && CONNECTION_ERROR_RE.test(causeMsg))
        return true;
    return false;
}
export async function withConnectionRecovery(fn, opts = {}) {
    const { signal, onRetry, initialDelayMs = 5000, maxDelayMs = 60000, backoffFactor = 2, } = opts;
    let attempt = 0;
