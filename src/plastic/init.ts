 ...
 */
function initSourceRepo(projectRoot: string): void {
  if (existsSync(join(SOURCE_REPO_DIR, ".git"))) return; // already initialized

  const stockSrcDir = join(projectRoot, "src");
  if (!existsSync(stockSrcDir)) return; // no source to copy
  ...
  for (const f of ["package.json", "tsconfig.json", "models.json"]) {
    ...
    if (existsSync(src)) safeCopy(src, join(SOURCE_REPO_DIR, f));
  }

  // Initialize git repo with stock source as initial commit
  try {
    execSync(
      'git init && git add -A && git commit -m "PLASTIC: stock source" && git checkout -b feature/plastic-changes',
      { cwd: SOURCE_REPO_DIR, stdio: "ignore" },
    );
  } catch {
...
export async function init(): Promise<void> {
  ...
  const sourceMapLines = [
    ...
    "2. **Edit** (preferred): Use `edit_source` tool for surgical search-and-replace edits. Path relative to overlay/ (e.g. 'main.js'). Provide old_string (exact match) and new_string. Fast — only send the changed portion.",
    "3. **Write**: Use `write_source` tool for full file rewrites, or shell commands (sed, cat). All work — the export pipeline diffs the overlay against stock regardless of how changes were made.",
    "4. **Reboot**: Emit `@@reboot@@` — saves state, exits, runner restarts with your changes",
    "5. **Export**: Emit `@@export@@` or call `export_changes` to generate a unified patch at `~/.gro/plastic/changes.patch`",
    "",
    "### Example: surgical edit",
    "```",
    "edit_source({ path: 'memory/virtual-memory.js', old_string: 'minRecentPerLane: 4', new_string: 'minRecentPerLane: 2' })",
    "@@reboot@@",
    "```",
    "",
 ...
 */
export function commitToSourceRepo(relPath: string, message: string, isTs = true): void {
  if (!existsSync(join(SOURCE_REPO_DIR, ".git"))) return;
  ...
  try {
    execSync(
      `git add "${prefix}/${relPath}" && git diff --cached --quiet || git commit -m "${message.replace(/"/g, '\\"')}"`,
      { cwd: SOURCE_REPO_DIR, stdio: "ignore" },
    );
  } catch {
