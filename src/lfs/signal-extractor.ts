...
export interface LfsSignal {
  v: string;     // viseme code
  c: number;     // confidence (0–1)
  e: number;     // entropy (0–1, normalized)
  dt: number;    // ms since last token
  seq: number;   // sequence number
  c_vel: number; // confidence velocity
  e_vel: number; // entropy velocity
}
...
function tokenToVisemes(token: string): string[] {
...
export class SignalExtractor {
  ...
  private prevConfidence = 0.85;
  private prevEntropy = 0.1;

  /**
   * Extract LFS signal(s) from a token emission.
   * Returns one signal per viseme in the token text.
   */
  extract(token: string, logprobs?: LogprobData): LfsSignal[] {
    const now = Date.now();
    ...
    if (visemes.length === 0) visemes.push("SIL");
