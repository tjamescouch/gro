...
export interface RuntimeConfig {
  ...
  model: string;
  provider: string;
  thinkingLevel: number; // 0.0–1.0
  learnFilePath: string;
  pagesDir: string;
}
...
export class RuntimeConfigurationManager {
  ...
  setDriver(driver: ChatDriver): void {
    this.driver = driver;
  }
  ...
  async loadLearnedFacts(): Promise<void> {
    try {
  ...
  async learn(fact: string): Promise<void> {
    ...
    if (this.currentMemory) {
      ...
      this.currentMemory.setSystemPrompt(newPrompt);
  ...
  async swapMemory(newType: string): Promise<AgentMemory> {
    ...
    const oldMessages = this.currentMemory ? this.currentMemory.getMessages() : [];
    const oldPages = this.currentMemory?.getPages ? this.currentMemory.getPages() : [];
    ...
    // Restore pages if new memory supports them
    if (oldPages.length > 0 && typeof newMemory.loadPage === "function") {
      Logger.info(`Restoring ${oldPages.length} pages...`);
      for (const page of oldPages) {
        try {
          await newMemory.loadPage(page.id);
        } catch (err: any) {
          Logger.warn(`Failed to restore page ${page.id}: ${err.message}`);
        }
      }
    }
  ...
  adjustThinkingLevel(delta: number): void {
