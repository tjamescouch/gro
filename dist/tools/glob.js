...
export function globToolDefinition() {
    return {
        type: "function",
        function: {
            name: "Glob",
...
export function executeGlob(args) {
    ...
    try {
        ...
        try {
            ...
            output = execSync(`cd ${JSON.stringify(searchPath)} && git ls-files --cached --others --exclude-standard 2>/dev/null | grep -E '${globToRegex(pattern)}' | head -${MAX_RESULTS + 1}`, { encoding: "utf-8", timeout: DEFAULT_TIMEOUT, stdio: ["pipe", "pipe", "pipe"] }).trim();
        }
        catch {
            // Fall back to find with basic glob
            output = execSync(`find ${JSON.stringify(searchPath)} -type f -name ${JSON.stringify(extractBaseName(pattern))} 2>/dev/null | head -${MAX_RESULTS + 1}`, { encoding: "utf-8", timeout: DEFAULT_TIMEOUT, stdio: ["pipe", "pipe", "pipe"] }).trim();
        }
        if (!output) {
        ...
        if (truncated)
            header += ` (truncated to ${MAX_RESULTS}, more exist)`;
        header += `:`;
        return header + "\n" + result.join("\n");
    }
    catch (e) {
 ...
 */
function extractBaseName(pattern) {
