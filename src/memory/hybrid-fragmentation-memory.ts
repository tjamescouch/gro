...
import { FragmentationMemory, type FragmentationMemoryConfig } from "./fragmentation-memory.js";
import type { ContextPage } from "./virtual-memory.js";
import { Logger } from "../logger.js";

export interface HybridFragmentationMemoryConfig extends FragmentationMemoryConfig {
  /** Whether to summarize selected fragments (default: true) */
  summaryOnFragments?: boolean;
  /** Max tokens to pass to summarizer from sampled fragments (default: 5000) */
  summaryInputTokens?: number;
  /** LLM model for summarization (same as VirtualMemory summarizerModel) */
  summarizerModel?: string;
  /** LLM driver for summarization calls */
...
export class HybridFragmentationMemory extends FragmentationMemory {
  private summaryOnFragments: boolean;
  private summaryInputTokens: number;
  private summarizerModel: string;
  private summarizerDriver: ChatDriver | null;

  constructor(config: HybridFragmentationMemoryConfig = {}) {
    super(config);
  ...
  async createPageFromMessages(
    ...
    const { page, summary: fragmentedSummary } = await super.createPageFromMessages(
      messages,
      label,
      lane,
    );
    ...
    try {
      ...
      const response = await this.summarizerDriver.chat(
        [{ role: "user", from: "system", content: summarizerPrompt }],
        {
