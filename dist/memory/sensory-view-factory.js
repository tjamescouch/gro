...
export function createDefaultFactory() {
    ...
    factory.register({ name: "context", maxTokens: 800, width: 82, height: 40, enabled: true, updateMode: "every_turn", viewable: true }, (deps) => new ContextMapSource(deps.memory, { maxChars: Math.floor(800 * 2.8), maxLines: 40 }));
    factory.register({ name: "time", maxTokens: 200, width: 82, height: 22, enabled: true, updateMode: "every_turn", viewable: true }, () => new TemporalSource());
    factory.register({ name: "tasks", maxTokens: 150, width: 82, height: 12, enabled: false, updateMode: "every_turn", viewable: true }, () => new TaskSource());
    factory.register({ name: "social", maxTokens: 200, width: 82, height: 12, enabled: true, updateMode: "every_turn", viewable: true }, () => new SocialSource());
    factory.register({ name: "spend", maxTokens: 100, width: 82, height: 12, enabled: false, updateMode: "every_turn", viewable: true }, (deps) => new SpendSource(deps.spendMeter));
    factory.register({ name: "violations", maxTokens: 80, width: 82, height: 12, enabled: false, updateMode: "every_turn", viewable: true }, () => new ViolationsSource(null));
    factory.register({ name: "awareness", maxTokens: 120, width: 82, height: 10, enabled: true, updateMode: "every_turn", viewable: true }, () => new AwarenessSource());
    factory.register({ name: "config", maxTokens: 120, width: 82, height: 17, enabled: true, updateMode: "every_turn", viewable: true }, () => new ConfigSource());
    factory.register({ name: "self", maxTokens: 200, width: 82, height: 20, enabled: false, updateMode: "every_turn", viewable: true }, () => new SelfSource());
    return factory;
}
