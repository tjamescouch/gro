 ...
 *
 * Single-agent, headless, no terminal UI.
...
import { newSessionId, findLatestSession, loadSession, saveSession, ensureGroDir } from "./session.js";
...
function loadMcpServers(mcpConfigPaths: string[]): Record<string, any> {
...
function loadConfig(): GroConfig {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    else if (arg === "--max-thinking-tokens") { flags.maxThinkingTokens = args[++i]; }
    else if (arg === "--max-budget-usd") { flags.maxBudgetUsd = args[++i]; }
...
async function singleShot(
  ...
  if (cfg.sessionPersistence) {
    saveSession(sessionId, memory.messages(), {
