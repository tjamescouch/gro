...
import { AgentMemory, type CompactionHints, type VirtualMemoryStats } from "../lib/agent-memory.js";
...
import { PhantomBuffer, type PhantomSnapshot } from "../lib/phantom-buffer.js";
...
function smartTruncate(text: string, limit: number): string {
  ...
  if (tailSize <= 0) return text.slice(0, limit);
...
export class VirtualMemory extends AgentMemory {
  ...
  private unrefHistory: Set<string> = new Set();
  ...
  override setProvider(provider: string): void {
    ...
    if (!process.env.GRO_AVG_CHARS_PER_TOKEN) {
      const PROVIDER_CHARS_PER_TOKEN: Record<string, number> = {
        anthropic: 3.0,
        openai: 2.8,
        google: 2.9,
        xai: 2.8,
   ...
   */
  private flattenCompactedToolCalls(pagedToolCallIds?: Map<string, string>): void {
  ...
  protected async onAfterAdd(): Promise<void> {
    ...
    await this.runOnce(async () => {
      ...
      const pagedToolCallIds = new Map<string, string>();
      ...
      if (olderTool.length >= 2) {
        ...
        for (const m of olderTool) {
          if (m.tool_call_id) pagedToolCallIds.set(m.tool_call_id, page.id);
        }
        summaries.push({
          role: "system",
          from: "VirtualMemory",
   ...
   */
  importPage(page: ContextPage): void {
    if (this.pages.has(page.id)) return; // Already known
    this.savePage(page);
    this.savePageIndex();
  }
   ...
   */
  grepPages(pattern: string, options?: {
  ...
  private startBatchWorker(): void {
    ...
    const d = this.cfg.driver as any;
