...
function tryAmaro(tsSource: string): string | null {
  try {
...
function tryTypeScript(tsSource: string): string | null {
  try {
 ...
 */
export function transpileTS(tsSource: string): string {
  const fromAmaro = tryAmaro(tsSource);
  if (fromAmaro !== null) return fromAmaro;

  const fromTS = tryTypeScript(tsSource);
  if (fromTS !== null) return fromTS;

  throw new Error(
    "No TypeScript transpiler available. " +
