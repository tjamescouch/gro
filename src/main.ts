...
import type { AgentMemory, VirtualMemoryStats } from "./memory/lib/agent-memory.js";
...
import { SelfSource } from "./memory/sensory/self-source.js";
import { ViolationsSource } from "./memory/sensory/violations-source.js";
    ...
    const handleMarker = (marker: { name: string; arg: string }) => {
      if (marker.name === "model-change") {
        ...
        try {
          ...
          let existing = "";
          try { existing = readFileSync(learnFile, "utf-8"); } catch {}
    ...
    for (const tc of output.toolCalls) {
      ...
      try {
        if (fnName === "apply_patch") {
          ...
          result = handleReboot(fnArgs as { reason?: string }, () => {
