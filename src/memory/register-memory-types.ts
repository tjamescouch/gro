...
memoryRegistry.register({
  ...
  factory: async (config: MemoryFactoryConfig) => {
    ...
    return new VirtualMemory({
      ...
      compactionTriggerRatio: config.compactionTriggerRatio as number | undefined,
      compactionTargetRatio: config.compactionTargetRatio as number | undefined,
      maxBatchSize: config.maxBatchSize as number | undefined,
      maxConcurrentBatches: config.maxConcurrentBatches as number | undefined,
    });
  },
  defaults: {
...
memoryRegistry.register({
  ...
  factory: async (config: MemoryFactoryConfig) => {
    ...
    return new HNSWMemory({
      systemPrompt: config.systemPrompt,
      maxMessages: config.maxMessages as number | undefined,
      dimensions: config.dimensions as number | undefined,
      maxConnections: config.maxConnections as number | undefined,
      efConstruction: config.efConstruction as number | undefined,
      efSearch: config.efSearch as number | undefined,
    });
  },
  defaults: {
    maxMessages: 1000,
  ...
  parameters: [
    { name: "maxMessages", type: "number", description: "Max messages to store", defaultValue: 1000 },
    { name: "dimensions", type: "number", description: "Embedding dimensions", defaultValue: 384 },
    { name: "maxConnections", type: "number", description: "Max connections per node", defaultValue: 16 },
    { name: "efConstruction", type: "number", description: "Construction search width", defaultValue: 200 },
    { name: "efSearch", type: "number", description: "Search width", defaultValue: 50 },
  ],
});
