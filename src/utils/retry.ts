...
export const MAX_RETRIES: number = 3;       // default — overridden at runtime via env
export const RETRY_BASE_MS: number = 1000;  // default — overridden at runtime via env

function maxRetries(): number  { return parseInt(process.env.GRO_MAX_RETRIES   ?? String(MAX_RETRIES),   10); }
function retryBaseMs(): number { return parseInt(process.env.GRO_RETRY_BASE_MS ?? String(RETRY_BASE_MS), 10); }
...
export function retryDelay(attempt: number, retryAfterHeader?: string | null): number {
  if (retryAfterHeader) {
    ...
    if (!isNaN(seconds) && seconds > 0) return Math.min(seconds * 1000, MAX_RETRY_DELAY_MS);
  }
