...
import { SimpleMemory } from "./memory/simple/simple-memory.js";
...
import { SensoryMemory } from "./memory/sensory/sensory-memory.js";
import { TemporalSource } from "./memory/sensory/temporal-source.js";
import { ConfigSource } from "./memory/sensory/config-source.js";
import { tryCreateEmbeddingProvider } from "./memory/lib/embedding-provider.js";
import { PageSearchIndex } from "./memory/virtual/page-search-index.js";
import { SemanticRetrieval } from "./memory/virtual/semantic-retrieval.js";
import { BatchSummarizer } from "./memory/virtual/batch-summarizer.js";
...
import { loadModelConfig, resolveModelAlias, isKnownAlias, modelIdPrefixPattern, inferProvider, } from "./model-config.js";
 ...
 */
async function executeTurn(driver, memory, mcp, cfg, sessionId, violations, turnAbortSignal, approvalFn) {
...
async function interactive(cfg, driver, mcp, sessionId) {
    ...
    const approvalFn = cfg.autoApprove ? undefined
        : isTerminal ? createApprovalGate({ sessionAllowlist: new Set(), outputFormat: cfg.outputFormat })
            : undefined;
            ...
            if (process.env.GRO_PLASTIC) {
                ...
                if (isOverlayError) {
                    ...
                    try {
                        const { patchPath, fileCount } = exportChanges();
                        if (fileCount > 0)
                            Logger.info(`[PLASTIC] Exported ${fileCount} diff(s) to ${patchPath} before rollback`);
                    }
                    catch { /* best effort */ }
    ...
    process.on(sig, async () => {
        ...
        if (process.env.GRO_PLASTIC) {
            try {
                ...
                if (fileCount > 0)
                    Logger.info(C.gray(`exported ${fileCount} PLASTIC change(s) to changes.patch`));
            }
            catch { }
        }
        Logger.info(spendMeter.formatSummary());
