...
import type { MemoryMetricsSnapshot, LaneMetrics } from "./memory-metrics.js";
...
export class MemoryTuner {
  private readonly MISS_RATE_THRESHOLD = 0.25; // Warn if >25% references miss
  private readonly LOW_COMPRESSION_THRESHOLD = 2.0; // Warn if compression <2x
  private readonly HIGH_RECALL_THRESHOLD = 0.4; // Good if >40% evicted pages get recalled
  private readonly MIN_SAMPLE_SIZE = 10; // Need at least 10 references for meaningful analysis

  /**
   * Analyze metrics and generate tuning recommendations.
   */
  tune(snapshot: MemoryMetricsSnapshot): TuningResult {
   ...
   */
  apply(recommendations: TuningRecommendation[], cfg: any): string {
   ...
   */
  formatRecommendations(result: TuningResult): string {
    const lines: string[] = [];
