...
export interface FragmenterConfig {
  /** Base sampling rate (0.0–1.0), default: 0.2 */
  sampleRate?: number;
  /** Age weight exponent (higher = more aggressive age bias), default: 1.5 */
  ageWeightExponent?: number;
  /** Minimum messages per fragment, default: 1 */
  minFragmentSize?: number;
  /** Maximum messages per fragment, default: 20 */
  maxFragmentSize?: number;
}

export interface Fragmenter {
  fragment(messages: ChatMessage[], config?: FragmenterConfig): Fragment[];
}

const DEFAULTS: Required<FragmenterConfig> = {
  sampleRate: 0.2,
...
export class RandomSamplingFragmenter implements Fragmenter {
  fragment(messages: ChatMessage[], config?: FragmenterConfig): Fragment[] {
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      const effectiveRate = Math.min(
        0.5,
        cfg.sampleRate * Math.pow(normalizedAge, cfg.ageWeightExponent)
      );
    ...
    if (sampled.length < cfg.minFragmentSize && messages.length > 0) {
      ...
      for (let i = 0; i < messages.length; i += step) {
        sampled.push(messages[i]);
        if (sampled.length >= cfg.minFragmentSize) break;
      }
    }
  ...
  private makePreview(messages: ChatMessage[]): string {
    if (messages.length === 0) return "";
