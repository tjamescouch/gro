...
export async function init() {
    ...
    const sourceMapLines = [
        ...
        "2. **Write**: **ALWAYS use the `write_source` tool** (path relative to dist/, e.g. `version.js`). This tracks changes for export. Do NOT use shell commands (sed, cat, echo) to edit overlay files — those changes are invisible to the export pipeline and will be lost.",
        "3. **Reboot**: Emit `@@reboot@@` — saves state, exits, runner restarts with your changes",
        "4. **Export**: Emit `@@export@@` or call `export_changes` to generate a unified patch at `~/.gro/plastic/changes.patch`",
        "",
        "### Example: bump version",
        "```",
        "write_source({ path: 'version.js', content: 'export const GRO_VERSION = \"2.11.0\";\\n' })",
        "@@reboot@@",
        "```",
        "",
