...
export function isMousePassthrough() { return _mousePassthrough; }
...
// ─── Help bar content ─────────────────────────────────────────────────────────
const HELP_NORMAL = " {bold}Enter{/bold}: Send  {bold}Tab{/bold}: Focus  {bold}Esc{/bold}: Input" +
    "  {bold}↑↓/jk{/bold}: Scroll  {bold}Ctrl+M{/bold}: Copy Mode  {bold}Ctrl+V{/bold}: Paste  {bold}Ctrl+C{/bold}: Quit";
const HELP_PASSTHROUGH = " {bold}{yellow-fg}COPY MODE — select freely.  Ctrl+M or Esc to return.{/yellow-fg}{/bold}";
function setHelpBarContent(screen, passthrough) {
    const helpBar = screen.children?.find((c) => c.position?.bottom === 0 && c.position?.height === 1);
    if (helpBar) {
...
// ─── Global key bindings ──────────────────────────────────────────────────────
export function setupGlobalKeys(screen, focusables, inputBox, onQuit, onPaste) {
    let focusIndex = 0;
    // Quit
